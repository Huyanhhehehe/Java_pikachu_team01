...
public class MainFrame extends JFrame implements ActionListener, Runnable {
    ...
    private JPanel createControlPanel() {
        ...
        panelScoreAndTime.add(panelCenter, BorderLayout.CENTER); // SỬA: panelCenter thay vì panelCenter.add
        ...
        // SỬA PHẦN NÀY - Tạo button panel đúng cách
        JPanel buttonPanel = new JPanel(new GridLayout(2, 1, 5, 5)); // 2 rows, 1 column

        // Tạo và thêm nút New Game
        btnNewGame = createButton("New Game");
        buttonPanel.add(btnNewGame);

        // Tạo và thêm nút Reset Icons
        JButton btnResetIcons = createButton("Reset Icons");
        buttonPanel.add(btnResetIcons);

        panelControl.add(buttonPanel, BorderLayout.PAGE_END);
    ...
    int confirm = JOptionPane.showConfirmDialog(this, 
            "Are you sure you want to reset all remaining icons?\nThis will randomly rearrange them.",
            "Reset Icons", 
            JOptionPane.YES_NO_OPTION);
    
    if (confirm == JOptionPane.YES_OPTION) {
        // Gọi phương thức reset icons trong graphicsPanel
        if (graphicsPanel != null) {
            graphicsPanel.resetRemainingIcons();
            }
        }
    }
    @Override
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnNewGame) {
            ...
            // Xử lý cả nút Reset Icons và các nút khác
            String command = e.getActionCommand();
            if ("Reset Icons".equals(command)) {
                resetIcons();
            }
        }
    }
